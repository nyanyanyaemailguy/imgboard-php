...
define("TITLE", 'untitled');		//タイトル（<title>とTOP）
define("HOME",  './mituba.php');			//「ホーム」へのリンク
...
define("IDSEED", '12345');		//idの種
define("RESIMG", 1);		//レスに画像を貼る:1 貼らない:0
define("reCAPTCHA_SITE_KEY", ''); # reCAPTCHAサイトキー. 使わない場合 ''.
...
.grecaptcha-note { font-size: 10pt; }
.grecaptcha-note a:link { color: #800000; }
.grecaptcha-note a:visited { color: #800000; }
.grecaptcha-note a:hover { color: #800000; }
.grecaptcha-note a:active { color: #800000; }
</style>
<script>
	function l(e) {
...
[<a href="'.PHP_SELF.'?mode=admin">admin pannel</a>]
<p align=center>
...
function form(&$dat, $resno, $admin = "") {
  ...
  $dat.=$msg.'<center>
<form action="'.PHP_SELF.'" method="POST" enctype="multipart/form-data" id = "post-form" autocomplete="off">
...
<small>
<ul>
  <li>Supported file types: GIF, JPG, PNG Some browsers may not attach files correctly.</li>
  <li>The maximum post size is '.MAX_KB.' KB. Includes the sage function.</li>
  <li>Images exceeding '.MAX_W.' pixels in width and '.MAX_H.' pixels in height will be displayed at a reduced size.</li>
  <li>All posts violating public order and morals are prohibited. Politics is not allowed. (Except for /pol/ and /b/...)</li>
  '.( reCAPTCHA_SITE_KEY == '' ? '' :
  '<li class="grecaptcha-note">
    This site is protected by reCAPTCHA and the Google
    <a href="https://policies.google.com/privacy">Privacy Policy</a> and
    <a href="https://policies.google.com/terms">Terms of Service</a> apply.
  </li>').'
  <li>MathJax is supported. Use `<code>\(\)</code>` for inline and `<code>\[\]</code>` for block.</li>
</ul>
...
function updatelog($resno = 0) {
  ...
  if($resno){
    ...
    if(!$find) error("The article you are looking for cannot be found.");
  }
  $line = file(LOGFILE);
  ...
  for($i = $st; $i < $st+PAGE_DEF; $i++){
    ...
    if($ext && is_file($img)){
      ...
      if($w && $h){//サイズがある時
        if(@is_file(THUMB_DIR.$time.'s.jpg')){
          $imgsrc = "<small>Displaying thumbnails. Click to view the original size.</small><br><a href=\"".$src."\" target=_blank><img src=".THUMB_DIR.$time.'s.jpg'.
      ...
      $dat.="file name:<a href=\"$src\" target=_blank>$time$ext</a>-($size B)<br>$imgsrc";
    }
    // メイン作成
    ...
    $dat.="\n<blockquote>".$com."</blockquote>";
     ...
     if($lineindex[$no]-1 >= LOG_MAX*0.95){
      $dat.="<font color=\"#f00000\"><b>This thread is old and will be deleted soon.</b></font><br>\n";
     }
    ...
    if(!$resno){
     ...
     elseif($s>1){
      $dat.="<font color=\"#707070\">レス".
             ($s - 1)." message omitted. Click the reply button to read all messages.</font><br>\n";
     }
    }else{$s=1;}
    ...
    if($ext && is_file($img)){
      ...
      if($w && $h){//サイズがある時
        if(@is_file(THUMB_DIR.$time.'s.jpg')){
          $imgsrc = "<small>Thumbnail view</small><br><a href=\"".$src."\" target=_blank><img src=".THUMB_DIR.$time.'s.jpg'.
      " border=0 align=left width=$w height=$h hspace=20 alt=\"".$size." B\"></a>";
        }else{
          $imgsrc = "<a href=\"".$src."\" target=_blank><img src=".$src.
      " border=0 align=left width=$w height=$h hspace=20 alt=\"".$size." B\"></a>";
...
<input type=hidden name=mode value=usrdel>[Article Deletion][<input type=checkbox name=onlyimgdel value=on>Delete only the image]<br>
delete key<input type=password name=pwd size=8 maxlength=8 value="">
<input type=submit value="delete"></form></td></tr></table>';
    ...
    // 改ページ処理
      ...
      if($prev >= 0){
        ...
        $dat.="<input type=submit value=\"Previous Page\">";
        $dat.="</td></form>";
      ...
      if($p >= PAGE_DEF && count($tree) > $next){
        ...
        $dat.="<input type=submit value=\"next page\">";
        $dat.="</td></form>";
...
function regist($name,$email,$sub,$com,$url,$pwd,$upfile,$upfile_name,$resto){
  ...
  if( $upfile && file_exists($upfile) ){
    ...
    if(!is_file($dest)) error("Upload failed<br>The server may not support this.",$dest);
    $size = getimagesize($dest);
    if(!is_array($size)) error("Upload failed<br>We do not accept anything other than image files.",$dest);
    $chk = md5_of_file($dest);
    foreach($badfile as $value){if(preg_match("/^$value/",$chk)){
      error("Upload failed<br>The same image was found.",$dest); //拒絶画像
    }}
    chmod($dest,0666);
    $W = $size[0];
    $H = $size[1];
    ...
    switch ($size[2]) {
      ...
      default : $ext=".xxx";error("This format is not supported.",$dest);
    }
    ...
    $mes = "The upload of image $upfile_name was successful.<br><br>";
  }

  foreach ($badstring as $value) {
    if(preg_match("/$value/", $com) || preg_match("/$value/", $sub) || preg_match("/$value/", $name) || preg_match("/$value/", $email)){
      error("Rejected(str)",$dest);
    };
  }
  if($_SERVER["REQUEST_METHOD"] != "POST") error("Please refrain from posting inappropriate content.(post)", $dest);
  // フォーム内容をチェック
  if(!$name||preg_match("/^[ |　|]*$/",$name)) $name="";
  if(!$com||preg_match("/^[ |　|\t]*$/",$com)) $com="";
  if(!$sub||preg_match("/^[ |　|]*$/",$sub))   $sub=""; 

  if(!$resto&&!$textonly&&!is_file($dest)) error("No image available",$dest);
  ...
  if(strlen($com) > 1000) error("The text is too long!",$dest);
  if(strlen($name) > 100) error("The text is too long!",$dest);
  if(strlen($email) > 100) error("The text is too long!",$dest);
  if(strlen($sub) > 100) error("The text is too long!",$dest);
  if(strlen($resto) > 10) error("This is abnormal.",$dest);
  if(strlen($url) > 10) error("This is abnormal.",$dest);
  ...
  if (preg_match('/ne\.jp$/', $host)
      || preg_match('/ad\.jp$/', $host)
      || preg_match('/bbtec\.net$/', $host)
      || preg_match('/aol\.com$/', $host)
      || preg_match('/uu\.net$/', $host)
      || preg_match('/asahi-net\.or\.jp$/', $host)
      || preg_match('/rim\.or\.jp$/', $host)) {
    $pxck = "off";
  } else { $pxck = "on"; }

  if($pxck=="on" && PROXY_CHECK){
    if(proxy_connect('80') == 1){
      error("ERROR! Public PROXY restricted!! (80)",$dest);
    } elseif(proxy_connect('8080') == 1){
      error("ERROR! Public PROXY restricted!! (8080)",$dest);
    }
  }
  ...
  for($i=0;$i<$imax;$i++){
    ## ここでエラーが出ている。 strlen(null) が非推奨と言われている。
   ...
   if(RENZOKU && $pchk && $time - $ltime < RENZOKU)
    error("Please wait a while before posting again.",$dest);
  ...
  if($dest&&file_exists($dest)){
    ...
    for($i=0;$i<$imax;$i++){ //画像重複チェック
     ...
     if($chkp==$chk&&file_exists($path.$timep.$extp)){
      error("Upload failed<br>The same image already exists",$dest);
    }}
  }
  ...
  //クッキー保存
  // setcookie ("pwdc", $c_pass, time()+7*24*3600);  /* 1週間で期限切れ. パスワード保存クッキーは今時いらない。 */ 
...
function usrdel($no,$pwd){
  ...
  if(!$flag) error("The article you are looking for cannot be found, or your password is incorrect.");
}
/* パス認証 */
function valid($pass){
  if($pass && $pass != ADMIN_PASS) error("The password is incorrect.");

  head($dat);
  echo $dat;
  echo "[<a href=\"".PHP_SELF2."\">Return to the bulletin board</a>]\n";
  echo "[<a href=\"".PHP_SELF."\">Update the log</a>]\n";
  echo "<table width='100%'><tr><th bgcolor=#E08000>\n";
  echo "<font color=#FFFFFF>Management Mode</font>\n";
  echo "</th></tr></table>\n";
  echo "<p><form action=\"".PHP_SELF."\" method=POST>\n";
  // ログインフォーム
  if(!$pass){
    echo "<center><input type=radio name=admin value=del checked>Article Deletion ";
    echo "<input type=radio name=admin value=post>Administrator Post<p>";
    echo "<input type=hidden name=mode value=admin>\n";
    echo "<input type=password name=pass size=8>";
    echo "<input type=submit value=\" Authentication \"></form></center>\n";
    die("</body></html>");
  }
}
/* 管理者削除 */
function admindel($pass){
  ...
  echo "<center><P>Check the box next to the article you wish to delete, then press the delete button.\n";
  echo "<p><input type=submit value=\"Delete\">";
  echo "<input type=reset value=\"reset\">";
  echo "[<input type=checkbox name=onlyimgdel value=on>delete image]";
  echo "<P><table border=1 cellspacing=0>\n";
  echo "<tr bgcolor=6080f6><th>delete</th><th>Article No.</th><th>Posting Date</th><th>title</th>";
  echo "<th>user</th><th>comment</th><th>host</th><th>image<br>(Bytes)</th><th>md5</th>";
  echo "</tr>\n";
  $line = file(LOGFILE);
...
function init(){
  ...
  if(!is_writable(realpath("./")))error("Cannot write to the current directory<br>");
  foreach($chkfile as $value){
    if(!file_exists(realpath($value))){
      $fp = fopen($value, "w");
      set_file_buffer($fp, 0);
      if($value==LOGFILE)fputs($fp,"1,2025/08/10(月) 19:19,anonymous,,hello,hello,,,,,,,,\n");
      if($value==TREEFILE)fputs($fp,"1\n");
      fclose($fp);
      if(file_exists(realpath($value)))@chmod($value,0666);
    }
    if(!is_writable(realpath($value)))$err.=$value."←I can't write something like this.<br>";
    if(!is_readable(realpath($value)))$err.=$value."←I can't read this.<br>";
  }
  @mkdir(IMG_DIR,0777);@chmod(IMG_DIR,0777);
  if(!is_dir(realpath(IMG_DIR)))$err.=IMG_DIR."←There's nothing like this.<br>";
  if(!is_writable(realpath(IMG_DIR)))$err.=IMG_DIR."←I can't write something like this.<br>";
  if(!is_readable(realpath(IMG_DIR)))$err.=IMG_DIR."←I can't read this.<br>";
  if(USE_THUMB){
    @mkdir(THUMB_DIR,0777);@chmod(THUMB_DIR,0777);
    if(!is_dir(realpath(IMG_DIR)))$err.=THUMB_DIR."←There's nothing like this.<br>";
    if(!is_writable(realpath(THUMB_DIR)))$err.=THUMB_DIR."←I can't write something like this.<br>";
    if(!is_readable(realpath(THUMB_DIR)))$err.=THUMB_DIR."←I can't read this.<br>";
  }
  if($err)error($err);
}
